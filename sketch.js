...
x=createSprite(width/2, height/2, 30,30)
x.visible= false
}
...
if(mousePressedOver(x)) {
   fill("black")
   textSize(40)
   text("You fixed the rocket",500,300)
}

if(mousePressedOver(astro)) {
   fill("black")
   textSize(30)
   text("'WE CRASHED OUR ROCKET'", 400,300)
   fill("black")
   textSize(30)
   text("'ANSWER THE QUESTIONS TO RESTART THE ROCKET'", 400,400)
}
...
function p3() {
   astro=createSprite(300,400,10,10)
   astro.addImage(astroImg)
   astro.scale=0.1
}

function rockCreate() {
   rocket=createSprite(450,350,10,10)
   rocket.addImage(rocketImg)
   rocket.scale=0.6
   fire=createSprite(450, 500,10,10)
   fire.addImage(fireImg)
   fire.scale=0.6
   fire.mirrorY(-1)
}
